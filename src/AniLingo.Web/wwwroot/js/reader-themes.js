(() => {
    ...
    const applyTheme = theme => {
        ...
        shell.dataset.themeEffect =
            reduceMotion.matches ? "none" : (motion.effect || "none");
        ...
        shell.style.setProperty(
            "--reader-theme-text-backdrop",
            themeValue(appearance.textBackdrop ?? .12, state.themeTextBackdropStrength, 0, .75));
        shell.style.setProperty(
            "--reader-theme-tint",
    ...
    const updateParallax = () => {
        ...
        const strength = clamp(state?.themeParallaxStrength ?? 1, 0, 2);
        const base = selectedTheme.motion || {};
        const scroll = window.scrollY;
        const shift = speed => {
            const raw = -(scroll * Number(speed || 0) * strength);
            const wrapped = ((raw % 320) + 320) % 320 - 160;
            return wrapped.toFixed(2) + "px";
        };
